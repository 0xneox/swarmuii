...
export function useReferrals(): UseReferralsReturn {
  ...
  const verifyReferralCode = useCallback(async (code: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to verify referral code');
      }

      const { referrerId } = await response.json();
  ...
  const processReferralRewards = useCallback(async (userId: string, earningAmount: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to process referral rewards');
      }
